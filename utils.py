...
async def broadcast_messages(user_id, message):
    try:
        ...
        logging.warning(f"⚠️ FloodWait: Sleeping for {e.value} seconds for user {user_id}")
        await asyncio.sleep(e.value)
        return await broadcast_messages(user_id, message)

    except InputUserDeactivated:
        await db.delete_user(int(user_id))
        logging.info(f"🗑️ {user_id} - Account deleted. Removed from database.")
        return False, "Deleted"

    except UserIsBlocked:
        logging.info(f"🚫 {user_id} - User blocked the bot.")
        return False, "Blocked"

    except PeerIdInvalid:
        await db.delete_user(int(user_id))
        logging.info(f"⚠️ {user_id} - PeerIdInvalid. Removed from database.")
        return False, "Error"
